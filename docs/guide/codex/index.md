...
Fast pricing uses a model-specific multiplier only when one is published. GPT-6 Astra uses the [documented 2× API Fast rate](https://developers.openai.com/api/docs/models/gpt-6-astra). GPT-5.6 Sol, Terra, and Luna use the [documented 2× API Priority rate](https://learn.chatgpt.com/docs/agent-configuration/speed#fast-mode). These API rates differ from the 2.5× ChatGPT credit-consumption rate for GPT-6 Astra and GPT-5.6 Fast mode: ccusage's `costUSD` is an API-equivalent estimate, not a ChatGPT credit balance. When a model's Fast rate is unknown, ccusage reports standard pricing rather than assuming a multiplier, which may underestimate actual Fast usage.
