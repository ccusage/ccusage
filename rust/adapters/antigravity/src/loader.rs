...
/// Stream every blob column returned by one query.
fn for_each_blob<F>(
...
/// Derive a deterministic session identifier from a database path.
///
/// If the database has a generic basename like `conversation.db`, `session.db`,
/// or `db.db` (common in nested `brain/<sess_id>/` layouts), the parent
/// directory name is used as the session identity.
fn derive_session_id(db_path: &Path) -> Arc<str> {
    if let Some(stem) = db_path.file_stem() {
        let stem_str = stem.to_string_lossy();
        if matches!(
            stem_str.as_ref(),
            "conversation" | "session" | "conversations" | "db" | "state"
        ) {
            if let Some(parent) = db_path.parent().and_then(|p| p.file_name()) {
                return Arc::from(parent.to_string_lossy().as_ref());
            }
        }
        return Arc::from(stem_str.as_ref());
    }
...
fn collect_records(db_path: &Path, shared: &SharedArgs) -> Vec<UsageRecord> {
    let conversation_id = derive_session_id(db_path);
    ...
    let mut parsed_generations = Vec::new();

    for_each_blob(&connection, db_path, ANTIGRAVITY_GENERATION_QUERY, shared, |blob| {
        for generation in parse_generation_metadata(blob) {
            if let Some(model) = generation.model.as_deref() {
            ...
            parsed_generations.push(generation);
        }
    });
    ...
    for_each_blob(&connection, db_path, ANTIGRAVITY_STEP_QUERY, shared, |blob| {
...
"#,
    ...
    #[test]
    fn derives_distinct_session_ids_from_nested_conversation_databases() {
        let fixture = fs_fixture!({});
        let sess1 = fixture.path("brain/sess-1/conversation.db");
        let sess2 = fixture.path("brain/sess-2/conversation.db");
        let direct = fixture.path("conversations/3eb80a64.db");

        std::fs::create_dir_all(sess1.parent().unwrap()).unwrap();
        std::fs::create_dir_all(sess2.parent().unwrap()).unwrap();
        std::fs::create_dir_all(direct.parent().unwrap()).unwrap();

        assert_eq!(derive_session_id(&sess1).as_ref(), "sess-1");
        assert_eq!(derive_session_id(&sess2).as_ref(), "sess-2");
        assert_eq!(derive_session_id(&direct).as_ref(), "3eb80a64");
    }
}
