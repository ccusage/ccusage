...
OpenCode v2 forks copy the parent's `session_message` rows with new ids but the same `seq`. When `session_v2.fork_session_id` and `fork_boundary` resolve against the parent's rows, fork rows at or below the boundary `seq` are skipped as inherited history, and the fork's cumulative `session_v2` aggregate is never used as a fallback. Unresolvable boundaries and pre-`seq` schemas keep every row.
